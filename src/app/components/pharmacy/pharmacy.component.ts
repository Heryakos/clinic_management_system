...
import { ConfirmDispenseDialogComponent } from '../confirm-dispense-dialog/confirm-dispense-dialog.component';
...
export class PharmacyComponent implements OnInit, OnDestroy {
  ...
  outOfStockPrescriptions: any[] = [];
  outOfStockItemsMap: { [prescriptionID: number]: string[] } = {};
  ...
  onSearch(): void {
    ...
    this.dispensedPrescriptions = [];
    this.isActivePrescriptions = false;
    this.loadMedications();
    const cardNumberInput = this.searchPatientID.trim();
    if (!cardNumberInput) {
      this.searchError = 'Please enter a Card Number.';
      this.isSearching = false;
      return;
    }

    console.log('Searching for Card Number:', cardNumberInput);

    this.cardNumber = cardNumberInput;

    // First try getPrescriptionsByCardNumber
    this.medicalService.getPrescriptionsByCardNumber(this.cardNumber).subscribe(
      (prescriptions: any[]) => {
        ...
        if (prescriptions && prescriptions.length > 0) {
          ...
          this.setPatientDetailsFromPrescriptions(prescriptions);
        } else {
          console.log('No prescriptions found with getPrescriptionsByCardNumber, trying getPrescriptionspayrollID');
          this.medicalService.getPrescriptionspayrollID(this.cardNumber).subscribe(
            (payrollPrescriptions: any[]) => {
              ...
              this.setPatientDetailsFromPrescriptions(payrollPrescriptions || []);
            },
            error => {
  ...
  private setPatientDetailsFromPrescriptions(prescriptions: any[]): void {
    if (prescriptions && prescriptions.length > 0) {
      const firstPrescription = prescriptions[0];
      this.prescriptionForm.patchValue({
        patientID: firstPrescription.PatientID || firstPrescription.patientID || '',
        patientName: firstPrescription.PatientName || firstPrescription.patientName || firstPrescription.FullName || '',
        cardNo: this.cardNumber,
        age: firstPrescription.Age || firstPrescription.age || '',
        sex: firstPrescription.Gender || firstPrescription.gender || ''
      });
    }
  }

  private processPrescriptions(prescriptions: any[]): void {
    const mappedPrescriptions = prescriptions.map((p, index) => {
    ...
    this.activePrescriptions = mappedPrescriptions.filter(p => {
      const status = (p.Status || p.status || 'Unknown').toLowerCase();
      return status !== 'dispensed';
    });

    this.dispensedPrescriptions = mappedPrescriptions.filter(p => {
      const status = (p.Status || p.status || 'Unknown').toLowerCase();
      return status === 'dispensed';
    });
    //out of stock
    this.outOfStockPrescriptions = mappedPrescriptions.filter(p => {
      const status = (p.Status || p.status || '').toLowerCase();
      return status === 'outofstock';
    });

    // Populate out of stock items
    this.outOfStockPrescriptions.forEach(pres => {
      const details = this.prescriptionDetailsMap[pres.prescriptionID];
      if (details && details.MedicationName) {
        this.outOfStockItemsMap[pres.prescriptionID] = this.extractMedicationNames(details.MedicationName);
      }
    });
    console.log('Processed active prescriptions:', this.activePrescriptions);
    console.log('Processed dispensed prescriptions:', this.dispensedPrescriptions);

    if (mappedPrescriptions.length > 0) {
      const detailPromises = mappedPrescriptions.map(prescription => {
      ...
      Promise.all(detailPromises).then(() => {
        ...
        this.resolveMissingPrescriptionIds().then(() => {
          ...
          this.activePrescriptions.forEach(prescription => {
            const details = this.prescriptionDetailsMap[prescription.prescriptionID];
            if (details) {
  ...
  getOutOfStockItems(prescriptionID: number): string[] {
    return this.outOfStockItemsMap[prescriptionID] || [];
  }

  private async resolveMissingPrescriptionIds(): Promise<void> {
    return new Promise<void>((resolve) => {
      const hasTemp = this.activePrescriptions.some(p => typeof p.prescriptionID !== 'number') ||
        this.dispensedPrescriptions.some(p => typeof p.prescriptionID !== 'number');
      if (!hasTemp) {
        resolve();
        return;
      }
      ...
      this.medicalService.getPrescriptions().subscribe({
        next: (all: any[]) => {
          ...
          this.activePrescriptions = this.activePrescriptions.map(p => {
          ...
          this.dispensedPrescriptions = this.dispensedPrescriptions.map(p => {
            if (typeof p.prescriptionID === 'number' && p.prescriptionID > 0) {
              return p;
            }
            const number = p.prescriptionNumber || p.PrescriptionNumber;
  ...
  viewPrescriptionDetails(prescriptionID: number | string, event: Event): void {
    ...
    const prescription = [...this.prescriptions, ...this.activePrescriptions, ...this.dispensedPrescriptions].find(p => p.prescriptionID === prescriptionID);
    if (!prescription) {
    ...
    if (!details || !details.MedicationName) {
      ...
      this.dialog.open(PrescriptionPaperComponent, {
        ...
        data: {
          ...
          prescription,
          medicationDetails: '',
          dialogTitle: 'Prescription Preview'
        }
      });
      return;
    }
    ...
    if (this.isActivePrescriptions) {
      // In queue mode, auto-populate based on inventory stock
      const allMedications = this.extractMedicationNames(details.MedicationName || '');
      this.medicationDetailsWithStock = allMedications.map(m => {
        const item = this.roomSpecificItems.flatMap(c => c.items).find(i => i.itemName.toLowerCase() === m.toLowerCase());
        const isInStock = item ? item.currentStock > 0 : false;
        return { medication: m, isInStock };
      });
      console.log('Auto-populated medicationDetailsWithStock for queue mode:', this.medicationDetailsWithStock);
    }

    if (status === 'dispensed') {
      // For dispensed, show all medications without separation
      const medicationDetails = details.MedicationName;
      this.dialog.open(PrescriptionPaperComponent, {
        width: '800px',
        data: {
          cardNumber: prescription.CardNumber,
          ...
          dialogTitle: 'Dispensed Prescription Details'
        }
      });
      return;
    }

    // For active, separate based on stock
    ...
    medicationEntries.forEach((medicationEntry: string) => {
      ...
      const stockItem = this.medicationDetailsWithStock.find(item => item.medication.toLowerCase() === medicationName.toLowerCase());
    ...
    this.dialog.open(PrescriptionPaperComponent, {
      ...
      data: {
        cardNumber: prescription.CardNumber,
        prescription,
        medicationDetails: inStockMedications.join(', ') + (outOfStockMedications.length > 0 ? ', ' + outOfStockMedications.join(', ') : ''),
        dialogTitle: 'Prescription Preview'
      }
    });
  }
        ...
        const nonDispensed = all
          ...
          .map(p => {
            ...
            return {
              ...
              FullName: p.PatientName || p.patientName || p.FullName || `${p.firstName || ''} ${p.lastName || ''}`.trim() || 'Unknown',
  ...
  onDispenseClick(prescription: any, event: Event): void {
    ...
    if (!id) {
      this.showErrorMessage("Invalid prescription ID");
      return;
    }

    // Debug: log when we start
    console.log('Dispense clicked for prescription ID:', id);

    this.medicalService.getPrescriptionDetails(id).subscribe({
      next: async (details: any[]) => {
        console.log('Raw prescription details:', details);

        // If no MedicationName, fetch it from medications list or fallback
        const enrichedDetails = await Promise.all(
          details.map(async (item: any) => {
            const medId = Number(item.medicationID ?? item.MedicationID ?? 0);
            let medName = (item.MedicationName || '').trim();

            // Fallback: if no name from SP, look up from loaded medications
            if (!medName && medId > 0) {
              const med = this.medications.find(m => Number(m.medicationID) === medId);
              medName = med?.medicationName || `Medication ${medId}`;
            }
            ...
            if (medId <= 0) {
              return {
                ...
                isInStock: false,
                availableStock: 0,
                requestedQty: Number(item.Quantity || 1),
                debug: 'No valid MedicationID'
              };
            }

            // 1. Get inventory ItemID from mapping
            const link = await this.medicalService.getInventoryItemForMedication(medId).toPromise()
              .catch(err => {
                console.warn(`Mapping failed for med ${medId}:`, err);
                return { ItemID: null };
              });

            const realItemId = link?.ItemID ? Number(link.ItemID) : null;
            console.log(`Med ${medId} → Mapped ItemID:`, realItemId);

            let stockItem = null;

            // 2. Force number comparison + safety checks
            if (realItemId && this.roomSpecificItems?.length > 0) {
              const allItems = this.roomSpecificItems.flatMap(c => c.items || []);
              console.log('Total inventory items loaded:', allItems.length);

              stockItem = allItems.find(i => {
                const invId = Number(i.itemID);
                const match = invId === realItemId;
                if (match) console.log(`MATCH FOUND! ItemID ${invId} = ${realItemId}, Stock: ${i.currentStock}`);
                return match;
              });
            } else if (realItemId) {
              console.warn('roomSpecificItems is empty or not loaded yet!');
            }

            // 3. Strong name fallback (cleaned)
            if (!stockItem) {
              const cleanMedName = medName.toLowerCase().replace(/\s+/g, ' ').trim();
              stockItem = this.roomSpecificItems
                .flatMap(c => c.items || [])
                .find(i => {
                  const invName = (i.itemName || '').toLowerCase().replace(/\s+/g, ' ').trim();
                  return invName === cleanMedName;
                });
              if (stockItem) console.log(`Name fallback match: ${medName} → ${stockItem.itemName}, Stock: ${stockItem.currentStock}`);
            }

            const availableStock = stockItem?.currentStock ?? 0;
            const requested = Number(item.Quantity || item.quantity || 1) || 1;
            const isInStock = availableStock >= requested && availableStock > 0;
            ...
            return {
              ...
              debugInfo: {
                medId,
                medName,
                mappedItemId: realItemId,
                foundBy: stockItem ? (realItemId ? 'ID' : 'Name') : 'None',
                stockValue: availableStock,
                stockItemName: stockItem?.itemName || 'Not found',
                roomItemsLoaded: this.roomSpecificItems?.length || 0
              }
            };
          })
        );

        console.log('Enriched details sent to dialog:', JSON.stringify(enrichedDetails, null, 2));

        const dialogRef = this.dialog.open(ConfirmDispenseDialogComponent, {
          width: '600px',
          data: {
            prescription,
        ...
        dialogRef.afterClosed().subscribe(result => {
          ...
          const notes = result.notes ?? null;   // safe: null if undefined
        
          if (result.confirm) {
            this.dispensePrescription(id, notes).subscribe({
              next: () => {
                this.showSuccessMessage('Dispensed successfully!');
                this.reloadAfterAction();
              },
              error: (err) => {
                this.showErrorMessage('Dispense failed: ' + (err.error?.message || 'Unknown error'));
              }
            });
          } else if (result.acknowledgeOutOfStock) {
            this.acknowledgeOutOfStock(id, notes).subscribe({
              next: () => {
                this.showSuccessMessage('Marked as Out of Stock!');
                this.reloadAfterAction();
              },
              error: (err) => {
                this.showErrorMessage('Failed to mark out of stock: ' + (err.error?.message || 'Unknown error'));
              }
            });
          }
        });
      ...
      error: (err) => {
        console.error('Prescription details error:', err);
        this.showErrorMessage("Failed to load prescription details");
      }
    });
  }
  private reloadAfterAction(): void {
    this.isRefreshingAfterAction = true;
    ...
    // Give Angular time to update the arrays, then hide spinner
    setTimeout(() => {
      this.isRefreshingAfterAction = false;
      this.cdr.detectChanges();
    }, 500); // 500ms delay is usually enough
  }
// Change from: dispensePrescription(...): void
dispensePrescription(prescriptionID: number, outOfStockNotes?: string) {
  if (!this.pharmacistId) {
    this.showErrorMessage("Pharmacist ID not found");
    return EMPTY; // ✅ Observable that completes immediately
  }

  const model = {
    PharmacistID: this.pharmacistId,
    OutOfStockNotes: outOfStockNotes || null
  };

  return this.medicalService.dispensePrescription(prescriptionID, model);
}

acknowledgeOutOfStock(prescriptionID: number, notes?: string) {
  if (!this.pharmacistId) {
    this.showErrorMessage("Pharmacist ID not found");
    return EMPTY;
  }
  ...
  return this.medicalService.acknowledgeOutOfStock(prescriptionID, model);
}
  // onDispenseClick(prescription: any, event: Event): void {
  //   event.stopPropagation();
  //   const id = this.getValidPrescriptionId(prescription);
